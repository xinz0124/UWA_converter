...
程序根据文件内容自动区分 GLB 和 MP4。转换内容包括压缩码流、属性名、压缩扩展层级、相机标签、观看约束（角度/弧度、包围盒表达及重力坐标系旋转）及扩展声明。0.4 转为 1.0 时保留原有 `UWA_viewing_parameters`，同时生成 `UWA_viewing_constraints`，并在 `extensionsUsed` 和 `extensionsRequired` 中同时保留两个扩展声明。0.4 的行优先 `gravityCoordinateSystem` 旋转矩阵会在转为 1.0 时转换为 glTF `nodes[0].rotation`（四元数顺序为 `x, y, z, w`）；单位矩阵不写入 `rotation`。反向转换时会将四元数还原为行优先矩阵并移除 `nodes[0].rotation`，无有效旋转时使用单位矩阵。MP4 转换会同步更新 `idat`、`meta` box 大小和 `iloc` 中的 extent length，视频和音频数据不变。
